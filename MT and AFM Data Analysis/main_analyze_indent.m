...
%% load ASCII Bruker exported file
pa = 'C:\Users\...' %file folder path
...
[z_piezo_NM,bruker_readout,z_tip_NM,paras]=read_indentation_file_bruker(pfn);

% show data
show_indentation_data(z_piezo_NM,bruker_readout);

%% level_indentation_data
[z_piezo_NM_c,bruker_readout_adjusted_c,z_tip_NM_c] = level_indentation_data(z_piezo_NM,bruker_readout,z_tip_NM);
Displacement=z_piezo_NM_c{select_extend1_withdraw2}-z_tip_NM_c{select_extend1_withdraw2};      % convert and use extend data
Force=bruker_readout_adjusted_c{select_extend1_withdraw2};   % convert and use extend data

%% select select indentation roi, this ROI will be used to calculate Young's modulus
[sDisplacement,sForce,ind]=manual_select_line_roi(Displacement,Force,'select indentation roi',para.indent_data_length,'bruker');
